...
def calibrate_and_save(camera_index=0):
    ...
    while True:
        ...
        # Draw instructions and points on the frame
        ...
        if key == ord('q'):
            ...
            break # Exit loop to proceed to input robot coordinates
    ...
    def run_interactive_mode(self, camera_index=0):
        ...
        while True:
            ...
            # 27 is the ESC key code
                ...
                break
            # Check for left mouse button click event (specific to waitKeyEx)
            # This is a bit of a workaround since setMouseCallback is more robust but we want to simplify
            # For simplicity, we'll just print on any key press that is not 'q'
            # A more robust solution uses setMouseCallback again. Let's add it for quality.
            
            # Re-enabling mouse callback for a better experience
...
if __name__ == '__main__':
    ...
    # This will create the 'transform_matrix.npy' file.
    # You only need to run this once, or whenever you need to re-calibrate.
    # To skip calibration and use an existing file, you can comment out this line.
    print("### PART 1: CALIBRATION ###")
    calibrate_and_save()

    # Check if calibration was successful before proceeding
    if not os.path.exists(TRANSFORM_MATRIX_FILE):
        ...
        # This will automatically load the matrix file we just created.
        print("\n### PART 2: TESTING THE TRANSFORMER ###")
        transformer = CoordinateTransformer()

        # --- Test Mode 1: Programmatic Transformation ---
        print("\n--- Test Mode 1: Transform a specific pixel coordinate ---")
        # Let's pick an arbitrary pixel coordinate to test.
        test_pixel_coord = (320, 240) # e.g., center of a 640x480 frame
        ...
        # --- Test Mode 2: Interactive Transformation ---
